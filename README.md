...
서지 정보 오류가 존재할 경우 서적들의
해당 페이지는 저장하지 않고 넘어갑니다.
해당 등록번호는 바탕화면의 "서지정보 오류 목록" 텍스트 파일로 저장됩니다.
이는 사서분들이 직접 수정하여야 합니다.

프로그램으로 조작중인 크롬창을 닫지 말아주십시오. ( 단, 다른 크롬창에서의 작업이나, 다른 프로그램 작업은 자유롭습니다 )
웹 로딩 관련 문제로 첫 서지정보 수정시, 제목이 누락될 수 있으니 작업이 완료된후 확인하여 주십시오.
